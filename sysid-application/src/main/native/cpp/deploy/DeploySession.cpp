...
// roboRIO image version.
static constexpr std::string_view kImageFile =
    "/etc/natinst/share/scs_imagemetadata.ini";

static constexpr std::array<std::string_view, 1> kValidImageVersions = {
    "2023_v3.*"};
...
/**
 * Validates the roboRIO image version.
 *
 * @param session A reference to the active SSH session into the roboRIO.
 * @param logger  A reference to the logger.
 *
 * @return Whether the image version check was successful.
 */
static bool CheckRoboRIOImageVersion(SshSession& session, wpi::Logger& logger) {
  // Create the regex pattern.
  std::regex pattern{
      "^IMAGEVERSION\\s*=\\s*\\\"FRC_roboRIO2?_(\\d{4}_v\\d+(?:\\.\\d+)?)\\\""};

  // Get the contents of the image version file on the roboRIO.
  std::string contents = session.Execute(fmt::format("cat {}", kImageFile));

  // Get the image version from the file.
  ...
  for (auto&& str : strings) {
    ...
    if (std::regex_match(str.begin(), str.end(), m, pattern)) {
      ...
      WPI_DEBUG(logger, "roboRIO Image Version: {}", version);
